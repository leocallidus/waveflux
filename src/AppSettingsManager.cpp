...
QString normalizeYtDlpFormat(const QString &value)
{
    ...
    if (normalized == QStringLiteral("m4a")
        ...
        return normalized == QStringLiteral("vorbis") ? QStringLiteral("ogg") : normalized;
    }
    return QStringLiteral("mp3");
}
...
const QHash<QString, QString> &englishTexts()
{
    static const QHash<QString, QString> texts = {
        ...
        {QStringLiteral("main.refreshPlaylist"), QStringLiteral("Refresh Playlist")},
        ...
        {QStringLiteral("ytDlpImport.tabInput"), QStringLiteral("Source & Preview")},
        {QStringLiteral("ytDlpImport.tabQueue"), QStringLiteral("Queue")},
        {QStringLiteral("ytDlpImport.tabSettings"), QStringLiteral("Format & Output")},
        {QStringLiteral("ytDlpImport.tabReport"), QStringLiteral("Report & History")},
        {QStringLiteral("ytDlpImport.bulkAddUrls"), QStringLiteral("Bulk URLs...")},
        {QStringLiteral("ytDlpImport.bulkAddTitle"), QStringLiteral("Bulk URL Input")},
        {QStringLiteral("ytDlpImport.bulkAddPrompt"), QStringLiteral("Paste one or more audio/video/playlist URLs (one per line):")},
        ...
        {QStringLiteral("ytDlpImport.postProcessingSection"), QStringLiteral("Tags & Metadata")},
        {QStringLiteral("ytDlpImport.embedMetadata"), QStringLiteral("Embed Metadata")},
        {QStringLiteral("ytDlpImport.embedMetadataHint"), QStringLiteral("Write title, artist, album, and other available tags into the audio file.")},
        {QStringLiteral("ytDlpImport.embedThumbnail"), QStringLiteral("Embed Cover Art")},
        {QStringLiteral("ytDlpImport.embedThumbnailHint"), QStringLiteral("Embed video or track thumbnail as audio album artwork.")},
        {QStringLiteral("ytDlpImport.cropCoverArt"), QStringLiteral("Crop Cover to Square")},
        {QStringLiteral("ytDlpImport.cropCoverArtHint"), QStringLiteral("Crop wide video thumbnails to a 1:1 square aspect ratio before embedding.")},
        {QStringLiteral("ytDlpImport.removeSourceMetadata"), QStringLiteral("Remove Source Identification Data")},
        {QStringLiteral("ytDlpImport.removeSourceMetadataHint"), QStringLiteral("Erase comments, descriptions, purl, and download URLs from metadata tags.")},
        {QStringLiteral("ytDlpImport.downloaderSection"), QStringLiteral("Downloader Engine & Acceleration")},
        {QStringLiteral("ytDlpImport.useAria2c"), QStringLiteral("Use aria2c Multi-Connection Downloader")},
        {QStringLiteral("ytDlpImport.useAria2cHint"), QStringLiteral("Alternative multi-connection downloader that can speed up downloads in certain regions.")},
        {QStringLiteral("ytDlpImport.aria2cConnections"), QStringLiteral("Max Server Connections (-x)")},
        {QStringLiteral("ytDlpImport.aria2cMinSplitSize"), QStringLiteral("Min Split Fragment Size (MiB)")},
        {QStringLiteral("ytDlpImport.customArgsSection"), QStringLiteral("Custom CLI Arguments")},
        {QStringLiteral("ytDlpImport.probeCustomArgs"), QStringLiteral("Detection Arguments (Probe)")},
        {QStringLiteral("ytDlpImport.probeCustomArgsPlaceholder"), QStringLiteral("e.g. --cookies-from-browser firefox --geo-bypass")},
        {QStringLiteral("ytDlpImport.downloadCustomArgs"), QStringLiteral("Loading Arguments (Download)")},
        {QStringLiteral("ytDlpImport.downloadCustomArgsPlaceholder"), QStringLiteral("e.g. --throttled-rate 100K --limit-rate 5M")},
        ...
        {QStringLiteral("playlist.refreshPlaylist"), QStringLiteral("Refresh Playlist")},
        ...
        {QStringLiteral("tagEditor.comment"), QStringLiteral("Comment:")},
        {QStringLiteral("tagEditor.composer"), QStringLiteral("Composer:")},
        {QStringLiteral("tagEditor.originalArtist"), QStringLiteral("Original Artist:")},
        {QStringLiteral("tagEditor.copyright"), QStringLiteral("Copyright:")},
        {QStringLiteral("tagEditor.url"), QStringLiteral("URL:")},
        {QStringLiteral("tagEditor.encoder"), QStringLiteral("Encoder:")},
        {QStringLiteral("tagEditor.tabTags"), QStringLiteral("Tags")},
        {QStringLiteral("tagEditor.tabCover"), QStringLiteral("Cover Art")},
        {QStringLiteral("tagEditor.tabChapters"), QStringLiteral("Chapters")},
        {QStringLiteral("tagEditor.tabTechInfo"), QStringLiteral("Track Info")},
        ...
        {QStringLiteral("tagEditor.coverExport"), QStringLiteral("Export Cover...")},
        {QStringLiteral("tagEditor.coverExportDialogTitle"), QStringLiteral("Save Cover Image")},
        {QStringLiteral("tagEditor.coverExportSuccess"), QStringLiteral("Cover image exported successfully")},
        {QStringLiteral("tagEditor.coverExportFailed"), QStringLiteral("Failed to export cover image: ")},
        {QStringLiteral("tagEditor.noCoverToExport"), QStringLiteral("No cover image available to export.")},
        ...
        {QStringLiteral("tagEditor.trackerWarning"), QStringLiteral("Warning: This track is a tracker module. Standard audio tags may not be supported or saved for tracker formats.")},
        {QStringLiteral("tagEditor.trackerBulkWarning"), QStringLiteral("Warning: Some selected tracks are tracker modules. Tag editing may not apply to them.")},
        {QStringLiteral("tagEditor.fileFormat"), QStringLiteral("File Format:")},
        {QStringLiteral("tagEditor.bitrate"), QStringLiteral("Bit Rate:")},
        {QStringLiteral("tagEditor.sampleRate"), QStringLiteral("Sample Rate:")},
        {QStringLiteral("tagEditor.channelMode"), QStringLiteral("Mode / Channels:")},
        {QStringLiteral("tagEditor.fileSize"), QStringLiteral("File Size:")},
        {QStringLiteral("tagEditor.duration"), QStringLiteral("Duration:")},
        {QStringLiteral("tagEditor.chapters"), QStringLiteral("Chapters")},
        {QStringLiteral("tagEditor.addChapter"), QStringLiteral("Add Chapter")},
        {QStringLiteral("tagEditor.editChapter"), QStringLiteral("Edit")},
        {QStringLiteral("tagEditor.deleteChapter"), QStringLiteral("Delete")},
        {QStringLiteral("tagEditor.clearChapters"), QStringLiteral("Clear All")},
        {QStringLiteral("tagEditor.chapterTitle"), QStringLiteral("Chapter Title")},
        {QStringLiteral("tagEditor.chapterStart"), QStringLiteral("Start Time (s)")},
        {QStringLiteral("tagEditor.chapterEnd"), QStringLiteral("End Time (s)")},
        {QStringLiteral("tagEditor.chapterDuration"), QStringLiteral("Duration")},
        {QStringLiteral("tagEditor.noChaptersYet"), QStringLiteral("No chapters in this track. Click 'Add Chapter' to create one.")},
        {QStringLiteral("tagEditor.sectionBasic"), QStringLiteral("Basic Information")},
        {QStringLiteral("tagEditor.sectionExtended"), QStringLiteral("Extended Metadata")},
        {QStringLiteral("tagEditor.sectionCover"), QStringLiteral("Cover Artwork")},
        {QStringLiteral("tagEditor.sectionTech"), QStringLiteral("Audio Specifications")},
        {QStringLiteral("tagEditor.selectAll"), QStringLiteral("Select All")},
        {QStringLiteral("tagEditor.clearSelection"), QStringLiteral("Clear Selection")},
        {QStringLiteral("audioConverter.title"), QStringLiteral("Audio Converter")},
        {QStringLiteral("audioConverter.tabFormat"), QStringLiteral("Format & Quality")},
        {QStringLiteral("audioConverter.tabProcessing"), QStringLiteral("Trim & DSP")},
        {QStringLiteral("audioConverter.tabPreview"), QStringLiteral("Live Simulation")},
        {QStringLiteral("audioConverter.tabInfo"), QStringLiteral("Source Info")},
        ...
        {QStringLiteral("audioConverter.resetDsp"), QStringLiteral("Reset DSP")},
        {QStringLiteral("audioConverter.listenSample"), QStringLiteral("Listen to Sample")},
        {QStringLiteral("audioConverter.stopSample"), QStringLiteral("Stop Sample")},
        {QStringLiteral("audioConverter.samplePlaying"), QStringLiteral("Playing sample preview...")},
        {QStringLiteral("audioConverter.sampleHint"), QStringLiteral("Preview the expected sound with current DSP and speed/pitch settings")},
        {QStringLiteral("audioConverter.sampleStart"), QStringLiteral("Sample start")},
        {QStringLiteral("audioConverter.sampleEnd"), QStringLiteral("Sample end")},
        {QStringLiteral("audioConverter.resetSampleRange"), QStringLiteral("Reset fragment")},
        {QStringLiteral("audioConverter.repeatSample"), QStringLiteral("Repeat fragment")},
        {QStringLiteral("audioConverter.repeatSampleHint"), QStringLiteral("Loop the preview fragment indefinitely")},
        ...
        {QStringLiteral("batchAudioConverter.tabQueue"), QStringLiteral("Queue")},
        {QStringLiteral("batchAudioConverter.tabFormat"), QStringLiteral("Format & Output")},
        {QStringLiteral("batchAudioConverter.tabDsp"), QStringLiteral("DSP & Enhancements")},
        {QStringLiteral("batchAudioConverter.tabReport"), QStringLiteral("Report & Logs")},
        ...
        {QStringLiteral("batchAudioConverter.clearQueue"), QStringLiteral("Clear Queue")},
        ...
        {QStringLiteral("batchAudioConverter.reportSection"), QStringLiteral("Batch Report & Status")},
        {QStringLiteral("batchAudioConverter.statTotal"), QStringLiteral("Total")},
        {QStringLiteral("batchAudioConverter.statSucceeded"), QStringLiteral("Succeeded")},
        {QStringLiteral("batchAudioConverter.statFailed"), QStringLiteral("Failed")},
        {QStringLiteral("batchAudioConverter.exportReport"), QStringLiteral("Export Report...")},
        ...
        {QStringLiteral("batchAudioConverter.start"), QStringLiteral("Start Conversion")},
        {QStringLiteral("batchAudioConverter.pause"), QStringLiteral("Pause")},
        {QStringLiteral("batchAudioConverter.resume"), QStringLiteral("Resume")},
        {QStringLiteral("batchAudioConverter.cancel"), QStringLiteral("Cancel")},
        {QStringLiteral("batchAudioConverter.close"), QStringLiteral("Close")},
        {QStringLiteral("batchAudioConverter.trimTitle"), QStringLiteral("Fragment Trimming")},
        {QStringLiteral("batchAudioConverter.trimEnable"), QStringLiteral("Trim fragment")},
        {QStringLiteral("batchAudioConverter.trimStart"), QStringLiteral("Start (s)")},
        {QStringLiteral("batchAudioConverter.trimEnd"), QStringLiteral("End (s)")},
        {QStringLiteral("batchAudioConverter.setTrim"), QStringLiteral("Set Fragment...")},
        {QStringLiteral("batchAudioConverter.batchSuccessNotice"), QStringLiteral("Batch conversion finished successfully! (%1 items)")},
        {QStringLiteral("batchAudioConverter.batchErrorNotice"), QStringLiteral("Batch completed with %1 error(s).")},
        {QStringLiteral("batchAudioConverter.batchCanceledNotice"), QStringLiteral("Batch conversion was canceled.")},
        ...
        {QStringLiteral("batchAudioConverter.statePaused"), QStringLiteral("Paused")},
        ...
        {QStringLiteral("settings.categoryGeneral"), QStringLiteral("General")},
        {QStringLiteral("settings.categoryGeneralDesc"), QStringLiteral("Language, startup, and session behavior")},
        {QStringLiteral("settings.categoryAppearance"), QStringLiteral("Appearance")},
        {QStringLiteral("settings.categoryAppearanceDesc"), QStringLiteral("Skin, typography, colors, and appearance defaults")},
        {QStringLiteral("settings.categoryPlaylist"), QStringLiteral("Playlist & Library")},
        {QStringLiteral("settings.categoryPlaylistDesc"), QStringLiteral("Layout, navigation, search, and file handling")},
        {QStringLiteral("settings.categoryPlayback"), QStringLiteral("Playback")},
        {QStringLiteral("settings.categoryPlaybackDesc"), QStringLiteral("Audio presentation, speed, repeat, shuffle, and seeking")},
        {QStringLiteral("settings.categoryWaveform"), QStringLiteral("Waveform")},
        {QStringLiteral("settings.categoryWaveformDesc"), QStringLiteral("Waveform size, interaction hints, and CUE overlays")},
        {QStringLiteral("settings.categoryTrackInfo"), QStringLiteral("Track Information")},
        {QStringLiteral("settings.categoryTrackInfoDesc"), QStringLiteral("Track display and formatting templates")},
        {QStringLiteral("settings.categorySystem"), QStringLiteral("System & Tools")},
        {QStringLiteral("settings.categorySystemDesc"), QStringLiteral("Tray, windows, updates, and external tools")},
        {QStringLiteral("settings.categoryShortcuts"), QStringLiteral("Keyboard Shortcuts")},
        {QStringLiteral("settings.categoryShortcutsDesc"), QStringLiteral("Shortcut discovery and assignment")},
        {QStringLiteral("settings.categoryAdvanced"), QStringLiteral("Advanced & Reset")},
        {QStringLiteral("settings.categoryAdvancedDesc"), QStringLiteral("Category, global, and factory reset actions")},
        {QStringLiteral("settings.groupLanguage"), QStringLiteral("Language")},
        {QStringLiteral("settings.groupStartup"), QStringLiteral("Startup & Resume")},
        {QStringLiteral("settings.groupCompletion"), QStringLiteral("Playback Completion")},
        {QStringLiteral("settings.groupInterfaceStyle"), QStringLiteral("Interface Style")},
        {QStringLiteral("settings.groupTypography"), QStringLiteral("Typography")},
        {QStringLiteral("settings.groupColors"), QStringLiteral("Colors")},
        {QStringLiteral("settings.groupAppearanceDefaults"), QStringLiteral("Appearance Defaults")},
        {QStringLiteral("settings.groupLayoutSidebars"), QStringLiteral("Layout & Sidebars")},
        {QStringLiteral("settings.groupColumnsRows"), QStringLiteral("Columns & Rows")},
        {QStringLiteral("settings.groupSearchNavigation"), QStringLiteral("Search & Navigation")},
        {QStringLiteral("settings.groupAddingOpening"), QStringLiteral("Adding and Opening Tracks")},
        {QStringLiteral("settings.groupFileOperations"), QStringLiteral("File Operations")},
        {QStringLiteral("settings.groupSpeedPitch"), QStringLiteral("Speed & Pitch")},
        {QStringLiteral("settings.groupAudioPresentation"), QStringLiteral("Audio Presentation")},
        {QStringLiteral("settings.groupFragmentRepeat"), QStringLiteral("Fragment Repeat")},
        {QStringLiteral("settings.groupShuffle"), QStringLiteral("Shuffle")},
        {QStringLiteral("settings.groupKeyboardSeeking"), QStringLiteral("Keyboard Seeking")},
        {QStringLiteral("settings.groupWaveformSize"), QStringLiteral("Size")},
        {QStringLiteral("settings.groupWaveformHints"), QStringLiteral("Interaction Hints")},
        {QStringLiteral("settings.groupCueOverlay"), QStringLiteral("CUE Overlay")},
        {QStringLiteral("settings.groupTrackInfoVisibility"), QStringLiteral("Visibility")},
        {QStringLiteral("settings.groupTrackInfoFormats"), QStringLiteral("Window and Tooltip Formats")},
        {QStringLiteral("settings.groupTrackInfoOverlayLayout"), QStringLiteral("Waveform Overlay Layout")},
        {QStringLiteral("settings.groupTrackInfoPreview"), QStringLiteral("Preview")},
        {QStringLiteral("settings.groupTrackInfoSyntax"), QStringLiteral("Format Syntax")},
        {QStringLiteral("settings.groupTrackInfoDefaults"), QStringLiteral("Track Information Defaults")},
        {QStringLiteral("settings.groupDesktopIntegration"), QStringLiteral("Desktop Integration")},
        {QStringLiteral("settings.groupWindowBehavior"), QStringLiteral("Window Behavior")},
        {QStringLiteral("settings.groupUpdates"), QStringLiteral("Updates")},
        {QStringLiteral("settings.groupExternalTools"), QStringLiteral("External Tools")},
        {QStringLiteral("settings.groupResetCategories"), QStringLiteral("Reset Categories")},
        {QStringLiteral("settings.groupResetAll"), QStringLiteral("Reset All Settings")},
        {QStringLiteral("settings.groupFactoryReset"), QStringLiteral("Factory Reset")},
        {QStringLiteral("settings.languageDescription"), QStringLiteral("Select application interface language.")},
        {QStringLiteral("settings.fontFamilyDescription"), QStringLiteral("Main font family used across the application.")},
        {QStringLiteral("settings.fontSizeDescription"), QStringLiteral("Base font size for interface elements.")},
        {QStringLiteral("settings.playlistFontFamilyDescription"), QStringLiteral("Font family used for the playlist track table.")},
        {QStringLiteral("settings.waveformColorDescription"), QStringLiteral("Primary peak and amplitude color of the waveform.")},
        {QStringLiteral("settings.waveformBackgroundColorDescription"), QStringLiteral("Background area color behind the waveform.")},
        {QStringLiteral("settings.progressColorDescription"), QStringLiteral("Played portion progress overlay color.")},
        {QStringLiteral("settings.accentColorDescription"), QStringLiteral("Accent highlight color for buttons, toggles, and selection.")},
        {QStringLiteral("settings.resetTheme"), QStringLiteral("Reset appearance to default")},
        {QStringLiteral("settings.resetThemeDescription"), QStringLiteral("Revert fonts and color scheme to default system values.")},
        {QStringLiteral("settings.sidebarPlaylistsSectionTitle"), QStringLiteral("Show playlists block")},
        ...
        {QStringLiteral("settings.speedDescription"), QStringLiteral("Playback speed multiplier.")},
        {QStringLiteral("settings.showSpeedPitchDescription"), QStringLiteral("Show speed and pitch adjust strip in the bottom control bar.")},
        {QStringLiteral("settings.audioQualityProfileDescription"), QStringLiteral("Audio decoding and resampler quality profile.")},
        {QStringLiteral("settings.displayVolumeInDecibelsDescription"), QStringLiteral("Show volume readout in decibels (dBFS) instead of percentages.")},
        {QStringLiteral("settings.dynamicSpectrumDescription"), QStringLiteral("Enable real-time audio spectrum visualization.")},
        {QStringLiteral("settings.fragmentRepeatDescription"), QStringLiteral("Enable A-B fragment loop playback.")},
        {QStringLiteral("settings.persistFragmentLoopPerTrackDescription"), QStringLiteral("Save and restore fragment loop boundaries per track.")},
        {QStringLiteral("settings.deterministicShuffleDescription"), QStringLiteral("Shuffle playlist using a reproducible pseudorandom sequence.")},
        {QStringLiteral("settings.shuffleSeedDescription"), QStringLiteral("Pseudorandom seed value for deterministic shuffle.")},
        {QStringLiteral("settings.repeatableShuffleDescription"), QStringLiteral("Maintain the same shuffled order on track restart.")},
        {QStringLiteral("settings.keyboardSeekStepSecondsDescription"), QStringLiteral("Step size in seconds when seeking with Left/Right arrow keys.")},
        {QStringLiteral("settings.keyboardSeekBackwardToPreviousTrackDescription"), QStringLiteral("Seeking backward at the start of a track jumps to previous track.")},
        {QStringLiteral("settings.waveformHeightDescription"), QStringLiteral("Height in pixels of the normal skin waveform.")},
        {QStringLiteral("settings.compactWaveformHeightDescription"), QStringLiteral("Height in pixels of the compact skin waveform.")},
        {QStringLiteral("settings.waveformZoomHintsVisibleDescription"), QStringLiteral("Show hover seek and zoom guide hints over the waveform.")},
        {QStringLiteral("settings.waveformCueOverlayEnabledDescription"), QStringLiteral("Display CUE sheet chapter markers over the waveform.")},
        {QStringLiteral("settings.waveformCueLabelsVisibleDescription"), QStringLiteral("Show chapter titles and time badges on the CUE overlay.")},
        {QStringLiteral("settings.waveformCueAutoHideOnZoomDescription"), QStringLiteral("Hide CUE markers automatically when zooming in on the waveform.")},
        {QStringLiteral("settings.trackInfoEnabledDescription"), QStringLiteral("Enable custom metadata overlays and title formatting.")},
        {QStringLiteral("settings.trackInfoWaveformOverlayHoverOnlyDescription"), QStringLiteral("Show waveform metadata overlays only when the mouse is over the waveform.")},
        {QStringLiteral("settings.trackInfoWindowTitleFormatDescription"), QStringLiteral("Template for the main window title bar.")},
        {QStringLiteral("settings.trackInfoWaveformTooltipFormatDescription"), QStringLiteral("Template for the waveform hover tooltip.")},
        {QStringLiteral("settings.trackInfoOverlayFormatsDescription"), QStringLiteral("9-position grid layout over the waveform.")},
        {QStringLiteral("settings.trackInfoPreviewDescription"), QStringLiteral("Live preview of formatted templates using current or sample track.")},
        {QStringLiteral("settings.trackInfoSyntaxDescription"), QStringLiteral("Reference guide for template formatting tokens and conditional blocks.")},
        {QStringLiteral("settings.trackInfoResetMinimalDescription"), QStringLiteral("Reset templates to minimal standard format.")},
        {QStringLiteral("settings.trackInfoClearAllDescription"), QStringLiteral("Clear all track information templates.")},
        {QStringLiteral("settings.resetAudioActionTitle"), QStringLiteral("Reset playback & audio")},
        {QStringLiteral("settings.resetAudioDescription"), QStringLiteral("Reset speed, pitch, shuffle, and audio presentation settings.")},
        {QStringLiteral("settings.resetWaveformActionTitle"), QStringLiteral("Reset waveform")},
        {QStringLiteral("settings.resetWaveformDescription"), QStringLiteral("Reset waveform heights, zoom hints, and CUE overlay settings.")},
        {QStringLiteral("settings.resetTrackInfoActionTitle"), QStringLiteral("Reset track information")},
        {QStringLiteral("settings.resetTrackInfoDescription"), QStringLiteral("Reset all metadata templates and overlay display settings.")},
        {QStringLiteral("settings.resetThemeActionTitle"), QStringLiteral("Reset appearance & theme")},
        {QStringLiteral("settings.resetThemeDescription"), QStringLiteral("Reset colors and font families to system defaults.")},
        {QStringLiteral("settings.resetAllActionTitle"), QStringLiteral("Reset all settings")},
        {QStringLiteral("settings.resetAllDescription"), QStringLiteral("Revert all settings across all categories to default values.")},
        {QStringLiteral("settings.shortcutResetAllDescription"), QStringLiteral("Restore all keyboard shortcuts to their factory default keybindings.")},
        {QStringLiteral("settings.searchResultsTitle"), QStringLiteral("Search Results")},
        {QStringLiteral("settings.searchNoResults"), QStringLiteral("No settings matched your query.")},
        {QStringLiteral("settings.clearSearch"), QStringLiteral("Clear search")},
        {QStringLiteral("settings.resetCategoryAction"), QStringLiteral("Reset this category")},
        {QStringLiteral("settings.resetSettingAction"), QStringLiteral("Reset this setting")},
        {QStringLiteral("settings.categoriesMenu"), QStringLiteral("Categories")},
        {QStringLiteral("settings.openDrawer"), QStringLiteral("Open category list")},
        {QStringLiteral("settings.closeDrawer"), QStringLiteral("Close category list")},
        {QStringLiteral("settings.modifiedBadge"), QStringLiteral("Modified")},
        ...
        {QStringLiteral("settings.recheckTool"), QStringLiteral("Recheck")},
        {QStringLiteral("settings.resetToAuto"), QStringLiteral("Reset to auto")},
        {QStringLiteral("settings.browseExecutable"), QStringLiteral("Browse...")},
        {QStringLiteral("settings.dependencyDisabledBecause"), QStringLiteral("Disabled because \"%1\" is turned off")},
        ...
        {QStringLiteral("settings.resetConfirmTitleSystem"), QStringLiteral("Confirm System Tools Reset")},
        {QStringLiteral("settings.resetConfirmTitleShortcuts"), QStringLiteral("Confirm Shortcuts Reset")},
        ...
        {QStringLiteral("settings.notifyOnTrackChange"), QStringLiteral("Notification when switching to the next track")},
        {QStringLiteral("settings.notifyOnTrackChangeDescription"), QStringLiteral("Show a notification for a few seconds with track details (title, artist, duration, or filename) when switching tracks.")},
        {QStringLiteral("notification.nowPlaying"), QStringLiteral("Now Playing")},
        {QStringLiteral("notification.nextTrack"), QStringLiteral("Next Track")},
        ...
        {QStringLiteral("dialogs.close"), QStringLiteral("Close")},
        {QStringLiteral("menu.settings"), QStringLiteral("Settings")},
        {QStringLiteral("menu.tools"), QStringLiteral("Tools")},
        {QStringLiteral("openUrl.cancel"), QStringLiteral("Cancel")},
        {QStringLiteral("player.noTrackLoaded"), QStringLiteral("No track loaded")},
        {QStringLiteral("settings.closeToTray"), QStringLiteral("Close to tray")},
        {QStringLiteral("settings.closeToTrayDescription"), QStringLiteral("Closing the window minimizes WaveFlux to the system tray.")},
        {QStringLiteral("settings.minimizeToTray"), QStringLiteral("Minimize to tray")},
        {QStringLiteral("settings.minimizeToTrayDescription"), QStringLiteral("Minimizing the window sends WaveFlux to the system tray.")},
        {QStringLiteral("settings.startMinimizedToTray"), QStringLiteral("Start minimized to tray")},
        {QStringLiteral("settings.startMinimizedToTrayDescription"), QStringLiteral("Start WaveFlux hidden in the system tray.")},
        {QStringLiteral("settings.compactPlaylistTrackNumberVisible"), QStringLiteral("Show track numbers in compact playlist")},
        {QStringLiteral("settings.compactPlaylistTrackNumberVisibleDescription"), QStringLiteral("Display track numbers when using compact playlist skin.")},
        {QStringLiteral("settings.confirmTrash"), QStringLiteral("Confirm moving files to trash")},
        {QStringLiteral("settings.confirmTrashDescription"), QStringLiteral("Ask for confirmation before sending playlist tracks to the trash.")},
        ...
        {QStringLiteral("dsp.generalTitle"), QStringLiteral("DSP & Audio Effects")},
        {QStringLiteral("dsp.managerTitle"), QStringLiteral("DSP & Audio Processing")},
        {QStringLiteral("dsp.managerDescription"), QStringLiteral("Fine-tune sound coloration, equalization, dynamics, and playback mixing")},
        {QStringLiteral("dsp.trackerWarningTitle"), QStringLiteral("Tracker module is playing")},
        {QStringLiteral("dsp.trackerWarningMessage"), QStringLiteral("Real-time DSP effects, equalization, and mixing are not supported for tracker modules. Convert the module to FLAC or WAV using the Audio Converter for full DSP functionality.")},
        {QStringLiteral("dsp.openAudioConverter"), QStringLiteral("Convert Module...")},
        {QStringLiteral("dsp.tabGeneral"), QStringLiteral("General")},
        {QStringLiteral("dsp.tabEq"), QStringLiteral("EQ")},
        {QStringLiteral("dsp.tabVolume"), QStringLiteral("Volume")},
        {QStringLiteral("dsp.tabMix"), QStringLiteral("Mix")},
        {QStringLiteral("dsp.tabSilenceRemoval"), QStringLiteral("Silence Removal")},
        {QStringLiteral("dsp.resetCurrentTab"), QStringLiteral("Reset Current Tab")},
        {QStringLiteral("dsp.resetTab"), QStringLiteral("Reset Tab")},
        {QStringLiteral("dsp.resetAllDsp"), QStringLiteral("Reset All DSP...")},
        {QStringLiteral("dsp.resetAll"), QStringLiteral("Reset All")},
        {QStringLiteral("dialog.close"), QStringLiteral("Close")},
        {QStringLiteral("dsp.resetParam"), QStringLiteral("Reset to default")},
        {QStringLiteral("dsp.resetConfirmTitle"), QStringLiteral("Reset Confirmation")},
        {QStringLiteral("dsp.resetCurrentTabConfirm"), QStringLiteral("Reset all parameters on this tab to their defaults?")},
        {QStringLiteral("dsp.resetAllDspConfirm"), QStringLiteral("Reset all DSP parameters to neutral defaults? Your saved EQ presets will not be deleted.")},
        {QStringLiteral("dsp.general.adjustments"), QStringLiteral("Adjustments")},
        {QStringLiteral("dsp.general.playback"), QStringLiteral("Playback Behavior")},
        {QStringLiteral("dsp.general.echo"), QStringLiteral("Echo")},
        {QStringLiteral("dsp.general.echoDesc"), QStringLiteral("Wet mix for delay effect")},
        {QStringLiteral("dsp.general.chorus"), QStringLiteral("Chorus")},
        {QStringLiteral("dsp.general.chorusDesc"), QStringLiteral("Wet mix for chorus modulation")},
        {QStringLiteral("dsp.general.speed"), QStringLiteral("Speed")},
        {QStringLiteral("dsp.general.speedDesc"), QStringLiteral("Varispeed: duration and pitch change together")},
        {QStringLiteral("dsp.general.reverb"), QStringLiteral("Reverb")},
        {QStringLiteral("dsp.general.reverbDesc"), QStringLiteral("Wet mix for room reverberation")},
        {QStringLiteral("dsp.general.bass"), QStringLiteral("Bass")},
        {QStringLiteral("dsp.general.bassDesc"), QStringLiteral("Low-shelf boost/cut (composes with EQ)")},
        {QStringLiteral("dsp.general.tempo"), QStringLiteral("Tempo")},
        {QStringLiteral("dsp.general.tempoDesc"), QStringLiteral("Time stretch without pitch change")},
        {QStringLiteral("dsp.general.flanger"), QStringLiteral("Flanger")},
        {QStringLiteral("dsp.general.flangerDesc"), QStringLiteral("Wet mix for flanger effect")},
        {QStringLiteral("dsp.general.stereoWidth"), QStringLiteral("Stereo Width")},
        {QStringLiteral("dsp.general.stereoWidthDesc"), QStringLiteral("Stereo image widening (1.00× = original)")},
        {QStringLiteral("dsp.general.tonality"), QStringLiteral("Pitch (Tonality)")},
        {QStringLiteral("dsp.general.tonalityDesc"), QStringLiteral("Pitch shift in semitones without tempo change")},
        {QStringLiteral("dsp.general.voiceSuppression"), QStringLiteral("Voice suppression (stereo only)")},
        {QStringLiteral("dsp.general.voiceSuppressionDesc"), QStringLiteral("Center-channel vocal cancellation")},
        {QStringLiteral("dsp.general.fadePauseResume"), QStringLiteral("Fade sound on pause and resume")},
        {QStringLiteral("dsp.general.fadePauseResumeDesc"), QStringLiteral("Ramp audio smoothly when pausing or resuming playback")},
        {QStringLiteral("dsp.general.fadeTrackNavigation"), QStringLiteral("Fade sound when navigating tracks")},
        {QStringLiteral("dsp.general.fadeTrackNavigationDesc"), QStringLiteral("Apply a short envelope when skipping or selecting tracks")},
        {QStringLiteral("dsp.volume.volumeBalance"), QStringLiteral("Volume & Balance")},
        {QStringLiteral("dsp.volume.smoothChanges"), QStringLiteral("Smooth volume changes")},
        {QStringLiteral("dsp.volume.smoothChangesDesc"), QStringLiteral("Ramp volume smoothly to prevent clicks and abrupt jumps")},
        {QStringLiteral("dsp.volume.logarithmicControl"), QStringLiteral("Perceptual volume control")},
        {QStringLiteral("dsp.volume.logarithmicControlDesc"), QStringLiteral("Map volume slider perceptually (logarithmic curve)")},
        {QStringLiteral("dsp.volume.loudnessCompensation"), QStringLiteral("Loudness compensation")},
        {QStringLiteral("dsp.volume.loudnessCompensationDesc"), QStringLiteral("Equal-loudness curve compensation at lower listening volumes")},
        {QStringLiteral("dsp.volume.master"), QStringLiteral("Master volume")},
        {QStringLiteral("dsp.volume.balance"), QStringLiteral("Balance")},
        {QStringLiteral("dsp.volume.balanceDesc"), QStringLiteral("Left/right stereo balance panning")},
        {QStringLiteral("dsp.volume.amplitudeNormalization"), QStringLiteral("Amplitude Normalization")},
        {QStringLiteral("dsp.volume.amplitudeNormalizationDesc"), QStringLiteral("Automatic realtime signal peak leveling")},
        {QStringLiteral("dsp.volume.targetPeakLevel"), QStringLiteral("Target peak level")},
        {QStringLiteral("dsp.volume.targetPeakLevelDesc"), QStringLiteral("Maximum target peak amplitude")},
        {QStringLiteral("dsp.volume.preamp"), QStringLiteral("Preamp")},
        {QStringLiteral("dsp.volume.preampDesc"), QStringLiteral("Gain applied before normalization")},
        {QStringLiteral("dsp.volume.useTagValues"), QStringLiteral("Prefer supported tag values")},
        {QStringLiteral("dsp.volume.useTagValuesDesc"), QStringLiteral("Use embedded peak metadata when available")},
        {QStringLiteral("dsp.volume.replayGain"), QStringLiteral("ReplayGain Normalization")},
        {QStringLiteral("dsp.volume.replayGainDesc"), QStringLiteral("Perceptual loudness matching using ReplayGain metadata")},
        {QStringLiteral("dsp.volume.replayGainMode"), QStringLiteral("Gain mode")},
        {QStringLiteral("dsp.volume.replayGainModeDesc"), QStringLiteral("Select between track, album, or automatic gain")},
        {QStringLiteral("dsp.volume.modeAuto"), QStringLiteral("Auto")},
        {QStringLiteral("dsp.volume.modeTrack"), QStringLiteral("Track")},
        {QStringLiteral("dsp.volume.modeAlbum"), QStringLiteral("Album")},
        {QStringLiteral("dsp.volume.fallbackGain"), QStringLiteral("Default gain when unavailable")},
        {QStringLiteral("dsp.volume.fallbackGainDesc"), QStringLiteral("Gain applied when ReplayGain tags are not present")},
        {QStringLiteral("dsp.volume.analyzeOnTheFly"), QStringLiteral("Analyze when tags are missing")},
        {QStringLiteral("dsp.volume.analyzeOnTheFlyDesc"), QStringLiteral("Perform dynamic analysis if no ReplayGain tags exist")},
        {QStringLiteral("dsp.volume.replayGainPreamp"), QStringLiteral("ReplayGain preamp")},
        {QStringLiteral("dsp.volume.replayGainPreampDesc"), QStringLiteral("Additional gain applied on top of ReplayGain value")},
        {QStringLiteral("dsp.volume.useReplayGainTags"), QStringLiteral("Use ReplayGain tags")},
        {QStringLiteral("dsp.volume.useReplayGainTagsDesc"), QStringLiteral("Read gain information from file metadata")},
        {QStringLiteral("dsp.volume.tagSource"), QStringLiteral("Tag source")},
        {QStringLiteral("dsp.volume.tagSourceDesc"), QStringLiteral("Metadata source priority")},
        {QStringLiteral("dsp.volume.sourceAuto"), QStringLiteral("Auto")},
        {QStringLiteral("dsp.volume.sourceFile"), QStringLiteral("File")},
        {QStringLiteral("dsp.volume.sourceCue"), QStringLiteral("CUE sheet")},
        {QStringLiteral("dsp.volume.effectiveGain"), QStringLiteral("Effective gain:")},
        {QStringLiteral("dsp.volume.noReplayGain"), QStringLiteral("None (0.0 dB)")},
        {QStringLiteral("dsp.mix.progression"), QStringLiteral("Playback Progression")},
        {QStringLiteral("dsp.mix.autoAdvance"), QStringLiteral("Automatically advance to the next track")},
        {QStringLiteral("dsp.mix.autoAdvanceDesc"), QStringLiteral("Advance to the next playlist item on track completion")},
        {QStringLiteral("dsp.mix.enableMixing"), QStringLiteral("Enable transition mixing")},
        {QStringLiteral("dsp.mix.enableMixingDesc"), QStringLiteral("Apply crossfade or fade envelopes during track transitions")},
        {QStringLiteral("dsp.mix.manual"), QStringLiteral("Manual Transition")},
        {QStringLiteral("dsp.mix.manualDesc"), QStringLiteral("Applied on explicit Next, Previous, or track selection")},
        {QStringLiteral("dsp.mix.manualCrossfade"), QStringLiteral("Mix outgoing and incoming tracks")},
        {QStringLiteral("dsp.mix.manualCrossfadeDesc"), QStringLiteral("Overlap outgoing and incoming audio")},
        {QStringLiteral("dsp.mix.manualCrossfadeMs"), QStringLiteral("Overlap duration")},
        {QStringLiteral("dsp.mix.manualCrossfadeMsDesc"), QStringLiteral("Crossfade overlap length in milliseconds")},
        {QStringLiteral("dsp.mix.manualFadeOut"), QStringLiteral("Fade out outgoing track")},
        {QStringLiteral("dsp.mix.manualFadeOutDesc"), QStringLiteral("Ramp down volume of outgoing audio")},
        {QStringLiteral("dsp.mix.manualFadeOutMs"), QStringLiteral("Fade-out duration")},
        {QStringLiteral("dsp.mix.manualFadeOutMsDesc"), QStringLiteral("Outgoing fade duration in milliseconds")},
        {QStringLiteral("dsp.mix.manualFadeIn"), QStringLiteral("Fade in incoming track")},
        {QStringLiteral("dsp.mix.manualFadeInDesc"), QStringLiteral("Ramp up volume of incoming audio")},
        {QStringLiteral("dsp.mix.manualFadeInMs"), QStringLiteral("Fade-in duration")},
        {QStringLiteral("dsp.mix.manualFadeInMsDesc"), QStringLiteral("Incoming fade duration in milliseconds")},
        {QStringLiteral("dsp.mix.automatic"), QStringLiteral("Automatic Transition")},
        {QStringLiteral("dsp.mix.automaticDesc"), QStringLiteral("Applied when a track finishes naturally")},
        {QStringLiteral("dsp.mix.automaticMode"), QStringLiteral("Transition mode")},
        {QStringLiteral("dsp.mix.modeGapless"), QStringLiteral("Gapless / no added transition")},
        {QStringLiteral("dsp.mix.modePause"), QStringLiteral("Insert a pause")},
        {QStringLiteral("dsp.mix.modeCrossfade"), QStringLiteral("Mix outgoing and incoming tracks")},
        {QStringLiteral("dsp.mix.automaticPauseMs"), QStringLiteral("Pause duration")},
        {QStringLiteral("dsp.mix.automaticPauseMsDesc"), QStringLiteral("Silence duration inserted between tracks")},
        {QStringLiteral("dsp.mix.automaticCrossfadeMs"), QStringLiteral("Crossfade duration")},
        {QStringLiteral("dsp.mix.automaticCrossfadeMsDesc"), QStringLiteral("Overlap duration for automatic end-of-track transition")},
        {QStringLiteral("dsp.mix.automaticFadeOutMs"), QStringLiteral("Outgoing fade duration")},
        {QStringLiteral("dsp.mix.automaticFadeOutMsDesc"), QStringLiteral("Fade out length on automatic transition")},
        {QStringLiteral("dsp.mix.automaticFadeInMs"), QStringLiteral("Incoming fade duration")},
        {QStringLiteral("dsp.mix.automaticFadeInMsDesc"), QStringLiteral("Fade in length on automatic transition")},
        {QStringLiteral("dsp.silenceRemoval.detection"), QStringLiteral("Silence Detection & Removal")},
        {QStringLiteral("dsp.silenceRemoval.enable"), QStringLiteral("Remove silence")},
        {QStringLiteral("dsp.silenceRemoval.enableDesc"), QStringLiteral("Automatically skip inaudible silence intervals during playback")},
        {QStringLiteral("dsp.silenceRemoval.minimumDuration"), QStringLiteral("Minimum silence duration")},
        {QStringLiteral("dsp.silenceRemoval.minimumDurationDesc"), QStringLiteral("Silence is skipped only after signal remains quiet for this length")},
        {QStringLiteral("dsp.silenceRemoval.threshold"), QStringLiteral("Silence threshold")},
        {QStringLiteral("dsp.silenceRemoval.thresholdDesc"), QStringLiteral("Levels below this threshold are treated as silence")},
        {QStringLiteral("dsp.silenceRemoval.trimEdges"), QStringLiteral("Remove silence at track edges regardless of duration")},
        {QStringLiteral("dsp.silenceRemoval.trimEdgesDesc"), QStringLiteral("Trim leading and trailing silence even if shorter than the minimum duration")},
        {QStringLiteral("dsp.silenceRemoval.timelineNotice"), QStringLiteral("Source timeline is preserved for waveform, seeking, markers, and scrobbling.")},
        {QStringLiteral("dsp.eq.bassNotice"), QStringLiteral("Note: Bass on the General tab is an independent low shelf that composes with these EQ bands.")},
        {QStringLiteral("dsp.reason.available"), QStringLiteral("Available")},
        {QStringLiteral("dsp.reason.monoSource"), QStringLiteral("Unavailable for mono audio")},
        {QStringLiteral("dsp.reason.liveStream"), QStringLiteral("Unavailable for live streams")},
        {QStringLiteral("dsp.reason.notSupportedTracker"), QStringLiteral("Not supported by tracker engine")},
        {QStringLiteral("dsp.reason.notSupported"), QStringLiteral("Unavailable for this source")},
        {QStringLiteral("menu.dspManager"), QStringLiteral("DSP Manager...")},
        {QStringLiteral("player.dspManager"), QStringLiteral("DSP Manager")},
        {QStringLiteral("shortcut.openDspManager"), QStringLiteral("Open DSP Manager")},
        ...
        {QStringLiteral("equalizer.resetBands"), QStringLiteral("Reset Bands")},
        ...
        {QStringLiteral("equalizer.presets"), QStringLiteral("Presets")},
        {QStringLiteral("equalizer.noPresets"), QStringLiteral("No presets in this list")},
        ...
        {QStringLiteral("equalizer.builtInPresets"), QStringLiteral("Built-in")},
        {QStringLiteral("equalizer.user"), QStringLiteral("User")},
        {QStringLiteral("equalizer.userPresets"), QStringLiteral("User")},
        ...
        {QStringLiteral("equalizer.savePreset"), QStringLiteral("Save Preset")},
        ...
        {QStringLiteral("equalizer.exportAll"), QStringLiteral("Export All")},
        {QStringLiteral("equalizer.namePlaceholder"), QStringLiteral("Preset name")},
        {QStringLiteral("equalizer.newPresetNamePlaceholder"), QStringLiteral("New preset name")},
        {QStringLiteral("equalizer.newPresetName"), QStringLiteral("New preset name")},
        ...
        {QStringLiteral("menu.refreshPlaylist"), QStringLiteral("Refresh Playlist")},
...
const QHash<QString, QString> &russianTexts()
{
    static const QHash<QString, QString> texts = {
        ...
        {QStringLiteral("main.refreshPlaylist"), QStringLiteral("Обновить плейлист")},
        ...
        {QStringLiteral("ytDlpImport.tabInput"), QStringLiteral("Источник и превью")},
        {QStringLiteral("ytDlpImport.tabQueue"), QStringLiteral("Очередь")},
        {QStringLiteral("ytDlpImport.tabSettings"), QStringLiteral("Формат и папка")},
        {QStringLiteral("ytDlpImport.tabReport"), QStringLiteral("Отчёт и журнал")},
        {QStringLiteral("ytDlpImport.bulkAddUrls"), QStringLiteral("Несколько ссылок...")},
        {QStringLiteral("ytDlpImport.bulkAddTitle"), QStringLiteral("Массовый ввод ссылок")},
        {QStringLiteral("ytDlpImport.bulkAddPrompt"), QStringLiteral("Вставьте ссылки на треки, видео или плейлисты (по одной на строку):")},
        ...
        {QStringLiteral("ytDlpImport.postProcessingSection"), QStringLiteral("Теги и метаданные")},
        {QStringLiteral("ytDlpImport.embedMetadata"), QStringLiteral("Встраивать метаданные")},
        {QStringLiteral("ytDlpImport.embedMetadataHint"), QStringLiteral("Записывать название, исполнителя, альбом и другие доступные теги в аудиофайл.")},
        {QStringLiteral("ytDlpImport.embedThumbnail"), QStringLiteral("Встраивать обложку")},
        {QStringLiteral("ytDlpImport.embedThumbnailHint"), QStringLiteral("Встраивать изображение превью видео или трека в качестве обложки альбома.")},
        {QStringLiteral("ytDlpImport.cropCoverArt"), QStringLiteral("Обрезать обложку до квадрата")},
        {QStringLiteral("ytDlpImport.cropCoverArtHint"), QStringLiteral("Обрезать широкие превью видео до квадратного соотношения 1:1 перед встраиванием.")},
        {QStringLiteral("ytDlpImport.removeSourceMetadata"), QStringLiteral("Удалять идентификационные данные источника")},
        {QStringLiteral("ytDlpImport.removeSourceMetadataHint"), QStringLiteral("Стирать поля комментариев, описания, purl и URL источника из тегов файла.")},
        {QStringLiteral("ytDlpImport.downloaderSection"), QStringLiteral("Движок загрузки и ускорение")},
        {QStringLiteral("ytDlpImport.useAria2c"), QStringLiteral("Использовать загрузчик aria2c")},
        {QStringLiteral("ytDlpImport.useAria2cHint"), QStringLiteral("Альтернативный многопоточный загрузчик, который может работать быстрее встроенного в ряде регионов.")},
        {QStringLiteral("ytDlpImport.aria2cConnections"), QStringLiteral("Макс. соединений к серверу (-x)")},
        {QStringLiteral("ytDlpImport.aria2cMinSplitSize"), QStringLiteral("Мин. размер фрагмента (МиБ)")},
        {QStringLiteral("ytDlpImport.customArgsSection"), QStringLiteral("Пользовательские аргументы CLI")},
        {QStringLiteral("ytDlpImport.probeCustomArgs"), QStringLiteral("Аргументы обнаружения (Probe)")},
        {QStringLiteral("ytDlpImport.probeCustomArgsPlaceholder"), QStringLiteral("напр. --cookies-from-browser firefox --geo-bypass")},
        {QStringLiteral("ytDlpImport.downloadCustomArgs"), QStringLiteral("Аргументы загрузки (Download)")},
        {QStringLiteral("ytDlpImport.downloadCustomArgsPlaceholder"), QStringLiteral("напр. --throttled-rate 100K --limit-rate 5M")},
        ...
        {QStringLiteral("playlist.refreshPlaylist"), QStringLiteral("Обновить плейлист")},
        ...
        {QStringLiteral("tagEditor.bpm"), QStringLiteral("Удары в минуту (BPM):")},
        {QStringLiteral("tagEditor.comment"), QStringLiteral("Комментарий:")},
        {QStringLiteral("tagEditor.composer"), QStringLiteral("Композитор:")},
        {QStringLiteral("tagEditor.originalArtist"), QStringLiteral("Исходный исполнитель:")},
        {QStringLiteral("tagEditor.copyright"), QStringLiteral("Авторские права:")},
        {QStringLiteral("tagEditor.url"), QStringLiteral("Ссылка (URL):")},
        {QStringLiteral("tagEditor.encoder"), QStringLiteral("Кодировщик:")},
        {QStringLiteral("tagEditor.tabTags"), QStringLiteral("Теги")},
        {QStringLiteral("tagEditor.tabCover"), QStringLiteral("Обложка")},
        {QStringLiteral("tagEditor.tabChapters"), QStringLiteral("Главы")},
        {QStringLiteral("tagEditor.tabTechInfo"), QStringLiteral("Информация")},
        ...
        {QStringLiteral("tagEditor.coverExport"), QStringLiteral("Экспорт обложки...")},
        {QStringLiteral("tagEditor.coverExportDialogTitle"), QStringLiteral("Сохранить обложку")},
        {QStringLiteral("tagEditor.coverExportSuccess"), QStringLiteral("Обложка успешно экспортирована")},
        {QStringLiteral("tagEditor.coverExportFailed"), QStringLiteral("Не удалось экспортировать обложку: ")},
        {QStringLiteral("tagEditor.noCoverToExport"), QStringLiteral("Нет изображения обложки для экспорта.")},
        ...
        {QStringLiteral("tagEditor.trackerWarning"), QStringLiteral("Внимание: Этот трек является трекерным модулем. Стандартные теги для него не поддерживаются или могут не сохраниться.")},
        {QStringLiteral("tagEditor.trackerBulkWarning"), QStringLiteral("Внимание: Некоторые выбранные файлы являются трекерными модулями. Изменения тегов могут к ним не примениться.")},
        {QStringLiteral("tagEditor.fileFormat"), QStringLiteral("Формат файла:")},
        {QStringLiteral("tagEditor.bitrate"), QStringLiteral("Битрейт:")},
        {QStringLiteral("tagEditor.sampleRate"), QStringLiteral("Частота дискретизации:")},
        {QStringLiteral("tagEditor.channelMode"), QStringLiteral("Каналы / Режим:")},
        {QStringLiteral("tagEditor.fileSize"), QStringLiteral("Размер файла:")},
        {QStringLiteral("tagEditor.duration"), QStringLiteral("Длительность:")},
        {QStringLiteral("tagEditor.chapters"), QStringLiteral("Главы")},
        {QStringLiteral("tagEditor.addChapter"), QStringLiteral("Добавить главу")},
        {QStringLiteral("tagEditor.editChapter"), QStringLiteral("Редактировать")},
        {QStringLiteral("tagEditor.deleteChapter"), QStringLiteral("Удалить")},
        {QStringLiteral("tagEditor.clearChapters"), QStringLiteral("Очистить все")},
        {QStringLiteral("tagEditor.chapterTitle"), QStringLiteral("Название главы")},
        {QStringLiteral("tagEditor.chapterStart"), QStringLiteral("Начало (сек)")},
        {QStringLiteral("tagEditor.chapterEnd"), QStringLiteral("Конец (сек)")},
        {QStringLiteral("tagEditor.chapterDuration"), QStringLiteral("Длительность")},
        {QStringLiteral("tagEditor.noChaptersYet"), QStringLiteral("В этом треке нет глав. Нажмите «Добавить главу», чтобы создать.")},
        {QStringLiteral("tagEditor.sectionBasic"), QStringLiteral("Основная информация")},
        {QStringLiteral("tagEditor.sectionExtended"), QStringLiteral("Дополнительные метаданные")},
        {QStringLiteral("tagEditor.sectionCover"), QStringLiteral("Обложка альбома")},
        {QStringLiteral("tagEditor.sectionTech"), QStringLiteral("Технические характеристики")},
        {QStringLiteral("tagEditor.selectAll"), QStringLiteral("Выбрать все")},
        {QStringLiteral("tagEditor.clearSelection"), QStringLiteral("Снять выделение")},
        {QStringLiteral("audioConverter.title"), QStringLiteral("Аудиоконвертер")},
        {QStringLiteral("audioConverter.tabFormat"), QStringLiteral("Формат и качество")},
        {QStringLiteral("audioConverter.tabProcessing"), QStringLiteral("Эффекты и обрезка")},
        {QStringLiteral("audioConverter.tabPreview"), QStringLiteral("Прослушивание")},
        {QStringLiteral("audioConverter.tabInfo"), QStringLiteral("Сведения о треке")},
        ...
        {QStringLiteral("audioConverter.resetDsp"), QStringLiteral("Сбросить DSP")},
        {QStringLiteral("audioConverter.listenSample"), QStringLiteral("Прослушать фрагмент")},
        {QStringLiteral("audioConverter.stopSample"), QStringLiteral("Остановить")},
        {QStringLiteral("audioConverter.samplePlaying"), QStringLiteral("Воспроизведение фрагмента...")},
        {QStringLiteral("audioConverter.sampleHint"), QStringLiteral("Предварительное прослушивание с текущими параметрами DSP и скорости/тона")},
        {QStringLiteral("audioConverter.sampleStart"), QStringLiteral("Начало фрагмента")},
        {QStringLiteral("audioConverter.sampleEnd"), QStringLiteral("Конец фрагмента")},
        {QStringLiteral("audioConverter.resetSampleRange"), QStringLiteral("Сбросить фрагмент")},
        {QStringLiteral("audioConverter.repeatSample"), QStringLiteral("Повторять фрагмент")},
        {QStringLiteral("audioConverter.repeatSampleHint"), QStringLiteral("Зациклить воспроизведение фрагмента")},
        ...
        {QStringLiteral("batchAudioConverter.tabQueue"), QStringLiteral("Очередь")},
        {QStringLiteral("batchAudioConverter.tabFormat"), QStringLiteral("Формат и вывод")},
        {QStringLiteral("batchAudioConverter.tabDsp"), QStringLiteral("Эффекты и DSP")},
        {QStringLiteral("batchAudioConverter.tabReport"), QStringLiteral("Отчёт и лог")},
        ...
        {QStringLiteral("batchAudioConverter.clearQueue"), QStringLiteral("Очистить очередь")},
        ...
        {QStringLiteral("batchAudioConverter.reportSection"), QStringLiteral("Отчет и статус партии")},
        {QStringLiteral("batchAudioConverter.statTotal"), QStringLiteral("Всего")},
        {QStringLiteral("batchAudioConverter.statSucceeded"), QStringLiteral("Успешно")},
        {QStringLiteral("batchAudioConverter.statFailed"), QStringLiteral("С ошибкой")},
        {QStringLiteral("batchAudioConverter.exportReport"), QStringLiteral("Экспорт отчета...")},
        ...
        {QStringLiteral("batchAudioConverter.start"), QStringLiteral("Начать конвертацию")},
        {QStringLiteral("batchAudioConverter.pause"), QStringLiteral("Пауза")},
        {QStringLiteral("batchAudioConverter.resume"), QStringLiteral("Продолжить")},
        {QStringLiteral("batchAudioConverter.cancel"), QStringLiteral("Отмена")},
        {QStringLiteral("batchAudioConverter.close"), QStringLiteral("Закрыть")},
        {QStringLiteral("batchAudioConverter.trimTitle"), QStringLiteral("Обрезка фрагмента")},
        {QStringLiteral("batchAudioConverter.trimEnable"), QStringLiteral("Обрезать фрагмент")},
        {QStringLiteral("batchAudioConverter.trimStart"), QStringLiteral("Начало (сек)")},
        {QStringLiteral("batchAudioConverter.trimEnd"), QStringLiteral("Конец (сек)")},
        {QStringLiteral("batchAudioConverter.setTrim"), QStringLiteral("Задать фрагмент...")},
        {QStringLiteral("batchAudioConverter.batchSuccessNotice"), QStringLiteral("Пакетная конвертация успешно завершена! (%1 элементов)")},
        {QStringLiteral("batchAudioConverter.batchErrorNotice"), QStringLiteral("Пакетная конвертация завершилась с ошибками (%1).")},
        {QStringLiteral("batchAudioConverter.batchCanceledNotice"), QStringLiteral("Пакетная конвертация была отменена.")},
        ...
        {QStringLiteral("batchAudioConverter.statePaused"), QStringLiteral("Приостановлено")},
        ...
        {QStringLiteral("settings.categoryGeneral"), QStringLiteral("Основные")},
        {QStringLiteral("settings.categoryGeneralDesc"), QStringLiteral("Язык интерфейса, запуск и параметры сессии")},
        {QStringLiteral("settings.categoryAppearance"), QStringLiteral("Внешний вид")},
        {QStringLiteral("settings.categoryAppearanceDesc"), QStringLiteral("Оформление, шрифты, цветовая схема и настройки по умолчанию")},
        {QStringLiteral("settings.categoryPlaylist"), QStringLiteral("Плейлист и медиатека")},
        {QStringLiteral("settings.categoryPlaylistDesc"), QStringLiteral("Макет, навигация, поиск и работа с файлами")},
        {QStringLiteral("settings.categoryPlayback"), QStringLiteral("Воспроизведение")},
        {QStringLiteral("settings.categoryPlaybackDesc"), QStringLiteral("Звук, скорость, повтор, перемешивание и перемотка")},
        {QStringLiteral("settings.categoryWaveform"), QStringLiteral("Волноформа")},
        {QStringLiteral("settings.categoryWaveformDesc"), QStringLiteral("Размер осциллограммы, подсказки и разметка CUE")},
        {QStringLiteral("settings.categoryTrackInfo"), QStringLiteral("Информация о треке")},
        {QStringLiteral("settings.categoryTrackInfoDesc"), QStringLiteral("Отображение метаданных и шаблоны форматирования")},
        {QStringLiteral("settings.categorySystem"), QStringLiteral("Система и утилиты")},
        {QStringLiteral("settings.categorySystemDesc"), QStringLiteral("Трей, поведение окон, обновления и внешние утилиты")},
        {QStringLiteral("settings.categoryShortcuts"), QStringLiteral("Горячие клавиши")},
        {QStringLiteral("settings.categoryShortcutsDesc"), QStringLiteral("Просмотр и переназначение комбинаций клавиш")},
        {QStringLiteral("settings.categoryAdvanced"), QStringLiteral("Дополнительно и сброс")},
        {QStringLiteral("settings.categoryAdvancedDesc"), QStringLiteral("Сброс разделов, всех настроек и полный сброс")},
        {QStringLiteral("settings.groupLanguage"), QStringLiteral("Язык")},
        {QStringLiteral("settings.groupStartup"), QStringLiteral("Запуск и возобновление")},
        {QStringLiteral("settings.groupCompletion"), QStringLiteral("Завершение воспроизведения")},
        {QStringLiteral("settings.groupInterfaceStyle"), QStringLiteral("Стиль интерфейса")},
        {QStringLiteral("settings.groupTypography"), QStringLiteral("Шрифты и типографика")},
        {QStringLiteral("settings.groupColors"), QStringLiteral("Цвета")},
        {QStringLiteral("settings.groupAppearanceDefaults"), QStringLiteral("Сброс оформления")},
        {QStringLiteral("settings.groupLayoutSidebars"), QStringLiteral("Макет и боковые панели")},
        {QStringLiteral("settings.groupColumnsRows"), QStringLiteral("Колонки и строки")},
        {QStringLiteral("settings.groupSearchNavigation"), QStringLiteral("Поиск и навигация")},
        {QStringLiteral("settings.groupAddingOpening"), QStringLiteral("Добавление и открытие файлов")},
        {QStringLiteral("settings.groupFileOperations"), QStringLiteral("Файловые операции")},
        {QStringLiteral("settings.groupSpeedPitch"), QStringLiteral("Скорость и тональность")},
        {QStringLiteral("settings.groupAudioPresentation"), QStringLiteral("Звуковое представление")},
        {QStringLiteral("settings.groupFragmentRepeat"), QStringLiteral("Повтор фрагмента (A-B)")},
        {QStringLiteral("settings.groupShuffle"), QStringLiteral("Перемешивание")},
        {QStringLiteral("settings.groupKeyboardSeeking"), QStringLiteral("Перемотка с клавиатуры")},
        {QStringLiteral("settings.groupWaveformSize"), QStringLiteral("Размер")},
        {QStringLiteral("settings.groupWaveformHints"), QStringLiteral("Интерактивные подсказки")},
        {QStringLiteral("settings.groupCueOverlay"), QStringLiteral("CUE Overlay")},
        {QStringLiteral("settings.groupTrackInfoVisibility"), QStringLiteral("Видимость")},
        {QStringLiteral("settings.groupTrackInfoFormats"), QStringLiteral("Форматы заголовка и подсказки")},
        {QStringLiteral("settings.groupTrackInfoOverlayLayout"), QStringLiteral("Разметка наложения осциллограммы")},
        {QStringLiteral("settings.groupTrackInfoPreview"), QStringLiteral("Предпросмотр")},
        {QStringLiteral("settings.groupTrackInfoSyntax"), QStringLiteral("Синтаксис шаблонов")},
        {QStringLiteral("settings.groupTrackInfoDefaults"), QStringLiteral("Сброс шаблонов информации о треке")},
        {QStringLiteral("settings.groupDesktopIntegration"), QStringLiteral("Интеграция с рабочим столом")},
        {QStringLiteral("settings.groupWindowBehavior"), QStringLiteral("Поведение окна")},
        {QStringLiteral("settings.groupUpdates"), QStringLiteral("Обновления")},
        {QStringLiteral("settings.groupExternalTools"), QStringLiteral("Внешние утилиты")},
        {QStringLiteral("settings.groupResetCategories"), QStringLiteral("Сброс разделов")},
        {QStringLiteral("settings.groupResetAll"), QStringLiteral("Сброс всех настроек")},
        {QStringLiteral("settings.groupFactoryReset"), QStringLiteral("Полный сброс приложения")},
        {QStringLiteral("settings.languageDescription"), QStringLiteral("Выберите язык пользовательского интерфейса.")},
        {QStringLiteral("settings.fontFamilyDescription"), QStringLiteral("Основное семейство шрифтов интерфейса приложения.")},
        {QStringLiteral("settings.fontSizeDescription"), QStringLiteral("Базовый размер шрифта интерфейса.")},
        {QStringLiteral("settings.playlistFontFamilyDescription"), QStringLiteral("Семейство шрифтов для строк таблицы плейлиста.")},
        {QStringLiteral("settings.waveformColorDescription"), QStringLiteral("Основной цвет амплитудных пиков осциллограммы.")},
        {QStringLiteral("settings.waveformBackgroundColorDescription"), QStringLiteral("Фоновый цвет подложки волноформы.")},
        {QStringLiteral("settings.progressColorDescription"), QStringLiteral("Цвет индикатора воспроизведённой части трека.")},
        {QStringLiteral("settings.accentColorDescription"), QStringLiteral("Акцентный цвет кнопок, переключателей и выделения.")},
        {QStringLiteral("settings.resetTheme"), QStringLiteral("Сбросить оформление по умолчанию")},
        {QStringLiteral("settings.resetThemeDescription"), QStringLiteral("Вернуть стандартные шрифты и системную палитру.")},
        {QStringLiteral("settings.sidebarPlaylistsSectionTitle"), QStringLiteral("Показывать блок плейлистов")},
        ...
        {QStringLiteral("settings.speedDescription"), QStringLiteral("Коэффициент скорости воспроизведения.")},
        {QStringLiteral("settings.showSpeedPitchDescription"), QStringLiteral("Отображать регуляторы скорости и тональности в панели управления.")},
        {QStringLiteral("settings.audioQualityProfileDescription"), QStringLiteral("Профиль качества декодирования и ресемплинга звука.")},
        {QStringLiteral("settings.displayVolumeInDecibelsDescription"), QStringLiteral("Отображать уровень громкости в децибелах (dBFS) вместо процентов.")},
        {QStringLiteral("settings.dynamicSpectrumDescription"), QStringLiteral("Включить динамическую визуализацию спектра частот.")},
        {QStringLiteral("settings.fragmentRepeatDescription"), QStringLiteral("Включить циклическое воспроизведение выбранного отрезка A-B.")},
        {QStringLiteral("settings.persistFragmentLoopPerTrackDescription"), QStringLiteral("Сохранять и восстанавливать границы фрагмента индивидуально для каждого трека.")},
        {QStringLiteral("settings.deterministicShuffleDescription"), QStringLiteral("Перемешивать плейлист по воспроизводимой псевдослучайной последовательности.")},
        {QStringLiteral("settings.shuffleSeedDescription"), QStringLiteral("Начальное число (seed) для детерминированного перемешивания.")},
        {QStringLiteral("settings.repeatableShuffleDescription"), QStringLiteral("Сохранять тот же порядок воспроизведения при перезапуске трека.")},
        {QStringLiteral("settings.keyboardSeekStepSecondsDescription"), QStringLiteral("Шаг перемотки в секундах клавишами стрелок влево/вправо.")},
        {QStringLiteral("settings.keyboardSeekBackwardToPreviousTrackDescription"), QStringLiteral("Перемотка назад в начале трека переходит к предыдущему треку.")},
        {QStringLiteral("settings.waveformHeightDescription"), QStringLiteral("Высота осциллограммы в пикселях в стандартном скине.")},
        {QStringLiteral("settings.compactWaveformHeightDescription"), QStringLiteral("Высота осциллограммы в пикселях в компактном скине.")},
        {QStringLiteral("settings.waveformZoomHintsVisibleDescription"), QStringLiteral("Отображать подсказки навигации и зума при наведении на волноформу.")},
        {QStringLiteral("settings.waveformCueOverlayEnabledDescription"), QStringLiteral("Отображать маркеры глав CUE-разметки на волноформе.")},
        {QStringLiteral("settings.waveformCueLabelsVisibleDescription"), QStringLiteral("Отображать названия и временные метки глав CUE.")},
        {QStringLiteral("settings.waveformCueAutoHideOnZoomDescription"), QStringLiteral("Автоматически скрывать маркеры CUE при масштабировании волноформы.")},
        {QStringLiteral("settings.trackInfoEnabledDescription"), QStringLiteral("Включить отображение пользовательских шаблонов метаданных.")},
        {QStringLiteral("settings.trackInfoWaveformOverlayHoverOnlyDescription"), QStringLiteral("Показывать информацию поверх волноформы только при наведении курсора.")},
        {QStringLiteral("settings.trackInfoWindowTitleFormatDescription"), QStringLiteral("Шаблон для заголовка главного окна программы.")},
        {QStringLiteral("settings.trackInfoWaveformTooltipFormatDescription"), QStringLiteral("Шаблон всплывающей подсказки при наведении на волноформу.")},
        {QStringLiteral("settings.trackInfoOverlayFormatsDescription"), QStringLiteral("Сетка из 9 позиций для размещения информации поверх волноформы.")},
        {QStringLiteral("settings.trackInfoPreviewDescription"), QStringLiteral("Предпросмотр шаблонов с использованием текущего или тестового трека.")},
        {QStringLiteral("settings.trackInfoSyntaxDescription"), QStringLiteral("Справочник тегов форматирования и условных конструкций.")},
        {QStringLiteral("settings.trackInfoResetMinimalDescription"), QStringLiteral("Сбросить шаблоны к минимальному стандартному виду.")},
        {QStringLiteral("settings.trackInfoClearAllDescription"), QStringLiteral("Очистить все шаблоны информации о треке.")},
        {QStringLiteral("settings.resetAudioActionTitle"), QStringLiteral("Сбросить звук и воспроизведение")},
        {QStringLiteral("settings.resetAudioDescription"), QStringLiteral("Сбросить скорость, тональность, перемешивание и параметры звука.")},
        {QStringLiteral("settings.resetWaveformActionTitle"), QStringLiteral("Сбросить волноформу")},
        {QStringLiteral("settings.resetWaveformDescription"), QStringLiteral("Сбросить размеры волноформы, подсказки и наложение CUE.")},
        {QStringLiteral("settings.resetTrackInfoActionTitle"), QStringLiteral("Сбросить информацию о треке")},
        {QStringLiteral("settings.resetTrackInfoDescription"), QStringLiteral("Сбросить шаблоны метаданных и настройки наложения.")},
        {QStringLiteral("settings.resetThemeActionTitle"), QStringLiteral("Сбросить оформление и тему")},
        {QStringLiteral("settings.resetThemeDescription"), QStringLiteral("Сбросить цвета и шрифты к системным значениям.")},
        {QStringLiteral("settings.resetAllActionTitle"), QStringLiteral("Сбросить все настройки")},
        {QStringLiteral("settings.resetAllDescription"), QStringLiteral("Вернуть все параметры во всех разделах к значениям по умолчанию.")},
        {QStringLiteral("settings.shortcutResetAllDescription"), QStringLiteral("Восстановить стандартные комбинации клавиш для всех команд.")},
        {QStringLiteral("settings.searchResultsTitle"), QStringLiteral("Результаты поиска")},
        {QStringLiteral("settings.searchNoResults"), QStringLiteral("По вашему запросу совпадений не найдено.")},
        {QStringLiteral("settings.clearSearch"), QStringLiteral("Очистить поиск")},
        {QStringLiteral("settings.resetCategoryAction"), QStringLiteral("Сбросить этот раздел")},
        {QStringLiteral("settings.resetSettingAction"), QStringLiteral("Сбросить эту настройку")},
        {QStringLiteral("settings.categoriesMenu"), QStringLiteral("Разделы")},
        {QStringLiteral("settings.openDrawer"), QStringLiteral("Открыть список разделов")},
        {QStringLiteral("settings.closeDrawer"), QStringLiteral("Закрыть список разделов")},
        {QStringLiteral("settings.modifiedBadge"), QStringLiteral("Изменено")},
        {QStringLiteral("settings.regenerateSeed"), QStringLiteral("Сгенерировать")},
        {QStringLiteral("settings.resetPitch"), QStringLiteral("Сбросить тон")},
        {QStringLiteral("settings.resetSpeed"), QStringLiteral("Сбросить скорость")},
        {QStringLiteral("settings.recheckTool"), QStringLiteral("Проверить")},
        {QStringLiteral("settings.resetToAuto"), QStringLiteral("Автоматически")},
        {QStringLiteral("settings.browseExecutable"), QStringLiteral("Обзор...")},
        {QStringLiteral("settings.dependencyDisabledBecause"), QStringLiteral("Отключено, так как выключена опция «%1»")},
        ...
        {QStringLiteral("settings.resetConfirmTitleGeneral"), QStringLiteral("Подтвердите сброс основных настроек")},
        {QStringLiteral("settings.resetConfirmTitleAppearance"), QStringLiteral("Подтвердите сброс внешнего вида")},
        {QStringLiteral("settings.resetConfirmTitlePlaylist"), QStringLiteral("Подтвердите сброс плейлиста")},
        ...
        {QStringLiteral("settings.resetConfirmTitleSystem"), QStringLiteral("Подтвердите сброс системных инструментов")},
        {QStringLiteral("settings.resetConfirmTitleShortcuts"), QStringLiteral("Подтвердите сброс горячих клавиш")},
        ...
        {QStringLiteral("settings.notifyOnTrackChange"), QStringLiteral("Уведомление при переключении на следующий трек")},
        {QStringLiteral("settings.notifyOnTrackChangeDescription"), QStringLiteral("Показывать уведомление на несколько секунд с информацией о треке (название, исполнитель, длительность или имя файла) при переключении треков.")},
        {QStringLiteral("notification.nowPlaying"), QStringLiteral("Сейчас играет")},
        {QStringLiteral("notification.nextTrack"), QStringLiteral("Следующий трек")},
        ...
        {QStringLiteral("dialogs.close"), QStringLiteral("Закрыть")},
        {QStringLiteral("menu.settings"), QStringLiteral("Настройки")},
        {QStringLiteral("menu.tools"), QStringLiteral("Инструменты")},
        {QStringLiteral("openUrl.cancel"), QStringLiteral("Отмена")},
        {QStringLiteral("player.noTrackLoaded"), QStringLiteral("Трек не загружен")},
        {QStringLiteral("settings.closeToTray"), QStringLiteral("Закрывать в трей")},
        {QStringLiteral("settings.closeToTrayDescription"), QStringLiteral("Закрытие окна сворачивает WaveFlux в системный трей.")},
        {QStringLiteral("settings.minimizeToTray"), QStringLiteral("Сворачивать в трей")},
        {QStringLiteral("settings.minimizeToTrayDescription"), QStringLiteral("Сворачивание окна прячет WaveFlux в системный трей.")},
        {QStringLiteral("settings.startMinimizedToTray"), QStringLiteral("Запускать свернутым в трей")},
        {QStringLiteral("settings.startMinimizedToTrayDescription"), QStringLiteral("Запуск WaveFlux в свернутом виде в системном трее.")},
        {QStringLiteral("settings.compactPlaylistTrackNumberVisible"), QStringLiteral("Номера треков в компактном плейлисте")},
        {QStringLiteral("settings.compactPlaylistTrackNumberVisibleDescription"), QStringLiteral("Отображать номера треков в компактном режиме плейлиста.")},
        {QStringLiteral("settings.confirmTrash"), QStringLiteral("Подтверждать перемещение в корзину")},
        {QStringLiteral("settings.confirmTrashDescription"), QStringLiteral("Запрашивать подтверждение перед удалением треков плейлиста в корзину.")},
        ...
        {QStringLiteral("dsp.generalTitle"), QStringLiteral("DSP и аудиоэффекты")},
        {QStringLiteral("dsp.managerTitle"), QStringLiteral("DSP-менеджер")},
        {QStringLiteral("dsp.managerDescription"), QStringLiteral("Эффекты воспроизведения, эквалайзер, обработка громкости, переходы и удаление тишины.")},
        {QStringLiteral("dsp.trackerWarningTitle"), QStringLiteral("Воспроизводится трекерный модуль")},
        {QStringLiteral("dsp.trackerWarningMessage"), QStringLiteral("DSP-эффекты, эквалайзер и микширование не поддерживаются для трекерных форматов. Рекомендуется конвертировать модуль в другой формат (FLAC/WAV) с помощью аудиоконвертера.")},
        {QStringLiteral("dsp.openAudioConverter"), QStringLiteral("Конвертировать...")},
        {QStringLiteral("dsp.tabGeneral"), QStringLiteral("Общие")},
        {QStringLiteral("dsp.tabEq"), QStringLiteral("Эквалайзер")},
        {QStringLiteral("dsp.tabVolume"), QStringLiteral("Громкость")},
        {QStringLiteral("dsp.tabMix"), QStringLiteral("Сведение")},
        {QStringLiteral("dsp.tabSilenceRemoval"), QStringLiteral("Удаление тишины")},
        {QStringLiteral("dsp.resetCurrentTab"), QStringLiteral("Сбросить текущую вкладку")},
        {QStringLiteral("dsp.resetTab"), QStringLiteral("Сбросить вкладку")},
        {QStringLiteral("dsp.resetAllDsp"), QStringLiteral("Сбросить все настройки DSP...")},
        {QStringLiteral("dsp.resetAll"), QStringLiteral("Сбросить всё")},
        {QStringLiteral("dialog.close"), QStringLiteral("Закрыть")},
        {QStringLiteral("dsp.resetParam"), QStringLiteral("Сбросить по умолчанию")},
        {QStringLiteral("dsp.resetConfirmTitle"), QStringLiteral("Подтверждение сброса")},
        {QStringLiteral("dsp.resetCurrentTabConfirm"), QStringLiteral("Сбросить все параметры на этой вкладке к значениям по умолчанию?")},
        {QStringLiteral("dsp.resetAllDspConfirm"), QStringLiteral("Сбросить все параметры DSP к нейтральным значениям? Сохраненные пресеты эквалайзера не будут удалены.")},
        {QStringLiteral("dsp.general.adjustments"), QStringLiteral("Регулировки")},
        {QStringLiteral("dsp.general.playback"), QStringLiteral("Поведение воспроизведения")},
        {QStringLiteral("dsp.general.echo"), QStringLiteral("Эхо")},
        {QStringLiteral("dsp.general.echoDesc"), QStringLiteral("Уровень эффекта задержки")},
        {QStringLiteral("dsp.general.chorus"), QStringLiteral("Хорус")},
        {QStringLiteral("dsp.general.chorusDesc"), QStringLiteral("Уровень модуляции хоруса")},
        {QStringLiteral("dsp.general.speed"), QStringLiteral("Скорость")},
        {QStringLiteral("dsp.general.speedDesc"), QStringLiteral("Скорость с пропорциональным изменением высоты тона")},
        {QStringLiteral("dsp.general.reverb"), QStringLiteral("Реверберация")},
        {QStringLiteral("dsp.general.reverbDesc"), QStringLiteral("Уровень пространственной реверберации")},
        {QStringLiteral("dsp.general.bass"), QStringLiteral("Бас")},
        {QStringLiteral("dsp.general.bassDesc"), QStringLiteral("Низкочастотная полка (суммируется с эквалайзером)")},
        {QStringLiteral("dsp.general.tempo"), QStringLiteral("Темп")},
        {QStringLiteral("dsp.general.tempoDesc"), QStringLiteral("Изменение темпа без изменения высоты тона")},
        {QStringLiteral("dsp.general.flanger"), QStringLiteral("Фленджер")},
        {QStringLiteral("dsp.general.flangerDesc"), QStringLiteral("Уровень эффекта фленджера")},
        {QStringLiteral("dsp.general.stereoWidth"), QStringLiteral("Стереобаза")},
        {QStringLiteral("dsp.general.stereoWidthDesc"), QStringLiteral("Расширение стереопанорамы (1.00× = исходная)")},
        {QStringLiteral("dsp.general.tonality"), QStringLiteral("Тональность")},
        {QStringLiteral("dsp.general.tonalityDesc"), QStringLiteral("Сдвиг высоты тона в полутонах без изменения темпа")},
        {QStringLiteral("dsp.general.voiceSuppression"), QStringLiteral("Подавление голоса (только стерео)")},
        {QStringLiteral("dsp.general.voiceSuppressionDesc"), QStringLiteral("Подавление центрального канала с вокалом")},
        {QStringLiteral("dsp.general.fadePauseResume"), QStringLiteral("Плавное затухание/нарастание при паузе и возобновлении")},
        {QStringLiteral("dsp.general.fadePauseResumeDesc"), QStringLiteral("Плавное изменение громкости при приостановке и продолжении воспроизведения")},
        {QStringLiteral("dsp.general.fadeTrackNavigation"), QStringLiteral("Плавный переход при переключении треков")},
        {QStringLiteral("dsp.general.fadeTrackNavigationDesc"), QStringLiteral("Короткое плавное затухание при ручном переключении треков")},
        {QStringLiteral("dsp.volume.volumeBalance"), QStringLiteral("Громкость и баланс")},
        {QStringLiteral("dsp.volume.smoothChanges"), QStringLiteral("Плавное изменение громкости")},
        {QStringLiteral("dsp.volume.smoothChangesDesc"), QStringLiteral("Плавное нарастание/спад громкости для предотвращения щелчков")},
        {QStringLiteral("dsp.volume.logarithmicControl"), QStringLiteral("Логарифмическая шкала громкости")},
        {QStringLiteral("dsp.volume.logarithmicControlDesc"), QStringLiteral("Логарифмическое распределение громкости по шкале")},
        {QStringLiteral("dsp.volume.loudnessCompensation"), QStringLiteral("Тонкомпенсация")},
        {QStringLiteral("dsp.volume.loudnessCompensationDesc"), QStringLiteral("Частотная коррекция для низких уровней громкости")},
        {QStringLiteral("dsp.volume.master"), QStringLiteral("Основная громкость")},
        {QStringLiteral("dsp.volume.balance"), QStringLiteral("Баланс")},
        {QStringLiteral("dsp.volume.balanceDesc"), QStringLiteral("Панорамирование между левым и правым каналами")},
        {QStringLiteral("dsp.volume.amplitudeNormalization"), QStringLiteral("Амплитудная нормализация")},
        {QStringLiteral("dsp.volume.amplitudeNormalizationDesc"), QStringLiteral("Автоматическое выравнивание пиковых уровней сигнала")},
        {QStringLiteral("dsp.volume.targetPeakLevel"), QStringLiteral("Целевой пиковый уровень")},
        {QStringLiteral("dsp.volume.targetPeakLevelDesc"), QStringLiteral("Максимальная целевая амплитуда пиков")},
        {QStringLiteral("dsp.volume.preamp"), QStringLiteral("Предусиление")},
        {QStringLiteral("dsp.volume.preampDesc"), QStringLiteral("Усиление перед нормализацией")},
        {QStringLiteral("dsp.volume.useTagValues"), QStringLiteral("Использовать значения из тегов при наличии")},
        {QStringLiteral("dsp.volume.useTagValuesDesc"), QStringLiteral("Использовать встроенные метаданные о пиках при их наличии")},
        {QStringLiteral("dsp.volume.replayGain"), QStringLiteral("Нормализация ReplayGain")},
        {QStringLiteral("dsp.volume.replayGainDesc"), QStringLiteral("Выравнивание громкости по стандарту ReplayGain")},
        {QStringLiteral("dsp.volume.replayGainMode"), QStringLiteral("Режим усиления")},
        {QStringLiteral("dsp.volume.replayGainModeDesc"), QStringLiteral("Выбор между трековым, альбомным или автоматическим усилением")},
        {QStringLiteral("dsp.volume.modeAuto"), QStringLiteral("Авто")},
        {QStringLiteral("dsp.volume.modeTrack"), QStringLiteral("Трек")},
        {QStringLiteral("dsp.volume.modeAlbum"), QStringLiteral("Альбом")},
        {QStringLiteral("dsp.volume.fallbackGain"), QStringLiteral("Усиление по умолчанию при отсутствии тегов")},
        {QStringLiteral("dsp.volume.fallbackGainDesc"), QStringLiteral("Усиление, используемое при отсутствии метаданных ReplayGain")},
        {QStringLiteral("dsp.volume.analyzeOnTheFly"), QStringLiteral("Анализировать при отсутствии тегов")},
        {QStringLiteral("dsp.volume.analyzeOnTheFlyDesc"), QStringLiteral("Выполнять динамический анализ, если теги отсутствуют")},
        {QStringLiteral("dsp.volume.replayGainPreamp"), QStringLiteral("Предусиление ReplayGain")},
        {QStringLiteral("dsp.volume.replayGainPreampDesc"), QStringLiteral("Дополнительное усиление поверх значений ReplayGain")},
        {QStringLiteral("dsp.volume.useReplayGainTags"), QStringLiteral("Использовать теги ReplayGain")},
        {QStringLiteral("dsp.volume.useReplayGainTagsDesc"), QStringLiteral("Считывать параметры усиления из метаданных файла")},
        {QStringLiteral("dsp.volume.tagSource"), QStringLiteral("Источник тегов")},
        {QStringLiteral("dsp.volume.tagSourceDesc"), QStringLiteral("Приоритет источника метаданных")},
        {QStringLiteral("dsp.volume.sourceAuto"), QStringLiteral("Авто")},
        {QStringLiteral("dsp.volume.sourceFile"), QStringLiteral("Файл")},
        {QStringLiteral("dsp.volume.sourceCue"), QStringLiteral("CUE-файл")},
        {QStringLiteral("dsp.volume.effectiveGain"), QStringLiteral("Текущее усиление:")},
        {QStringLiteral("dsp.volume.noReplayGain"), QStringLiteral("Нет (0.0 дБ)")},
        {QStringLiteral("dsp.mix.progression"), QStringLiteral("Порядок воспроизведения")},
        {QStringLiteral("dsp.mix.autoAdvance"), QStringLiteral("Автоматически переходить к следующему треку")},
        {QStringLiteral("dsp.mix.autoAdvanceDesc"), QStringLiteral("Переходить к следующей дорожке в списке по завершении трека")},
        {QStringLiteral("dsp.mix.enableMixing"), QStringLiteral("Включить сведение переходов")},
        {QStringLiteral("dsp.mix.enableMixingDesc"), QStringLiteral("Использовать эффекты плавного перехода между треками")},
        {QStringLiteral("dsp.mix.manual"), QStringLiteral("Ручной переход")},
        {QStringLiteral("dsp.mix.manualDesc"), QStringLiteral("Применяется при нажатии «Следующий», «Предыдущий» или выборе трека")},
        {QStringLiteral("dsp.mix.manualCrossfade"), QStringLiteral("Сводить уходящий и входящий треки")},
        {QStringLiteral("dsp.mix.manualCrossfadeDesc"), QStringLiteral("Накладывать воспроизведение уходящего и входящего треков")},
        {QStringLiteral("dsp.mix.manualCrossfadeMs"), QStringLiteral("Длительность наложения")},
        {QStringLiteral("dsp.mix.manualCrossfadeMsDesc"), QStringLiteral("Длительность перекрестного затухания в миллисекундах")},
        {QStringLiteral("dsp.mix.manualFadeOut"), QStringLiteral("Затухать уходящий трек")},
        {QStringLiteral("dsp.mix.manualFadeOutDesc"), QStringLiteral("Плавное снижение громкости уходящего трека")},
        {QStringLiteral("dsp.mix.manualFadeOutMs"), QStringLiteral("Длительность затухания")},
        {QStringLiteral("dsp.mix.manualFadeOutMsDesc"), QStringLiteral("Длительность затухания в миллисекундах")},
        {QStringLiteral("dsp.mix.manualFadeIn"), QStringLiteral("Нарастать входящий трек")},
        {QStringLiteral("dsp.mix.manualFadeInDesc"), QStringLiteral("Плавное нарастание громкости входящего трека")},
        {QStringLiteral("dsp.mix.manualFadeInMs"), QStringLiteral("Длительность нарастания")},
        {QStringLiteral("dsp.mix.manualFadeInMsDesc"), QStringLiteral("Длительность нарастания в миллисекундах")},
        {QStringLiteral("dsp.mix.automatic"), QStringLiteral("Автоматический переход")},
        {QStringLiteral("dsp.mix.automaticDesc"), QStringLiteral("Применяется при естественном завершении трека")},
        {QStringLiteral("dsp.mix.automaticMode"), QStringLiteral("Режим перехода")},
        {QStringLiteral("dsp.mix.modeGapless"), QStringLiteral("Без пауз / без добавления переходов")},
        {QStringLiteral("dsp.mix.modePause"), QStringLiteral("Вставить паузу")},
        {QStringLiteral("dsp.mix.modeCrossfade"), QStringLiteral("Сводить уходящий и входящий треки")},
        {QStringLiteral("dsp.mix.automaticPauseMs"), QStringLiteral("Длительность паузы")},
        {QStringLiteral("dsp.mix.automaticPauseMsDesc"), QStringLiteral("Длительность паузы тишины между треками")},
        {QStringLiteral("dsp.mix.automaticCrossfadeMs"), QStringLiteral("Длительность сведения")},
        {QStringLiteral("dsp.mix.automaticCrossfadeMsDesc"), QStringLiteral("Длительность наложения при автоматическом переходе")},
        {QStringLiteral("dsp.mix.automaticFadeOutMs"), QStringLiteral("Длительность затухания")},
        {QStringLiteral("dsp.mix.automaticFadeOutMsDesc"), QStringLiteral("Длительность затухания при автоматическом переходе")},
        {QStringLiteral("dsp.mix.automaticFadeInMs"), QStringLiteral("Длительность нарастания")},
        {QStringLiteral("dsp.mix.automaticFadeInMsDesc"), QStringLiteral("Длительность нарастания при автоматическом переходе")},
        {QStringLiteral("dsp.silenceRemoval.detection"), QStringLiteral("Обнаружение и удаление тишины")},
        {QStringLiteral("dsp.silenceRemoval.enable"), QStringLiteral("Удалять тишину")},
        {QStringLiteral("dsp.silenceRemoval.enableDesc"), QStringLiteral("Автоматически пропускать участки тишины во время воспроизведения")},
        {QStringLiteral("dsp.silenceRemoval.minimumDuration"), QStringLiteral("Минимальная длительность тишины")},
        {QStringLiteral("dsp.silenceRemoval.minimumDurationDesc"), QStringLiteral("Тишина пропускается только если длится не менее указанного времени")},
        {QStringLiteral("dsp.silenceRemoval.threshold"), QStringLiteral("Порог тишины")},
        {QStringLiteral("dsp.silenceRemoval.thresholdDesc"), QStringLiteral("Уровни ниже порога считаются тишиной")},
        {QStringLiteral("dsp.silenceRemoval.trimEdges"), QStringLiteral("Удалять тишину в начале и конце трека любой длительности")},
        {QStringLiteral("dsp.silenceRemoval.trimEdgesDesc"), QStringLiteral("Удалять тишину в начале и конце трека даже если она короче минимального интервала")},
        {QStringLiteral("dsp.silenceRemoval.timelineNotice"), QStringLiteral("Таймлайн исходного файла сохраняется для формы волны, поиска, маркеров и статистики.")},
        {QStringLiteral("dsp.eq.bassNotice"), QStringLiteral("Примечание: регулятор «Бас» на вкладке «Общие» — это независимая низкочастотная полка, которая суммируется с полосами эквалайзера.")},
        {QStringLiteral("dsp.reason.available"), QStringLiteral("Доступно")},
        {QStringLiteral("dsp.reason.monoSource"), QStringLiteral("Недоступно для монофонического аудио")},
        {QStringLiteral("dsp.reason.liveStream"), QStringLiteral("Недоступно для прямых трансляций")},
        {QStringLiteral("dsp.reason.notSupportedTracker"), QStringLiteral("Не поддерживается трекерным движком")},
        {QStringLiteral("dsp.reason.notSupported"), QStringLiteral("Недоступно для данного источника")},
        {QStringLiteral("menu.dspManager"), QStringLiteral("DSP-менеджер...")},
        {QStringLiteral("player.dspManager"), QStringLiteral("DSP-менеджер")},
        {QStringLiteral("shortcut.openDspManager"), QStringLiteral("Открыть DSP-менеджер")},
        ...
        {QStringLiteral("equalizer.resetBands"), QStringLiteral("Сбросить полосы")},
        ...
        {QStringLiteral("equalizer.presets"), QStringLiteral("Предустановки")},
        {QStringLiteral("equalizer.noPresets"), QStringLiteral("В этом списке нет пресетов")},
        ...
        {QStringLiteral("equalizer.builtInPresets"), QStringLiteral("Встроенные")},
        {QStringLiteral("equalizer.user"), QStringLiteral("Пользовательские")},
        {QStringLiteral("equalizer.userPresets"), QStringLiteral("Пользовательские")},
        ...
        {QStringLiteral("equalizer.savePreset"), QStringLiteral("Сохранить пресет")},
        ...
        {QStringLiteral("equalizer.exportAll"), QStringLiteral("Экспортировать все")},
        {QStringLiteral("equalizer.namePlaceholder"), QStringLiteral("Название пресета")},
        {QStringLiteral("equalizer.newPresetNamePlaceholder"), QStringLiteral("Имя нового пресета")},
        {QStringLiteral("equalizer.newPresetName"), QStringLiteral("Имя нового пресета")},
        ...
        {QStringLiteral("menu.refreshPlaylist"), QStringLiteral("Обновить плейлист")},
...
QVariantMap AppSettingsManager::validateYtDlpImportRuntime(const QString &selectedFormat)
{
    ...
    const bool requiresFfmpeg = normalizedFormat.isEmpty()
        ...
        || normalizedFormat == QStringLiteral("vorbis");
...
void AppSettingsManager::setNotifyOnTrackChange(bool enabled)
{
    if (m_notifyOnTrackChange == enabled) {
        return;
    }

    m_notifyOnTrackChange = enabled;
    emit notifyOnTrackChangeChanged();
    scheduleSaveSettings();
}
...
void AppSettingsManager::loadSettings()
{
    ...
    m_notifyOnTrackChange = m_settings.value("playback.notifyOnTrackChange", true).toBool();
...
void AppSettingsManager::saveSettings()
{
    ...
    m_settings.setValue("playback.notifyOnTrackChange", m_notifyOnTrackChange);
...
QVariantMap AppSettingsManager::normalizeYtDlpImportLastSettings(const QVariantMap &settings)
{
    ...
    normalized.insert(QStringLiteral("probeCustomArgs"),
                      settings.value(QStringLiteral("probeCustomArgs")).toString().trimmed());
    normalized.insert(QStringLiteral("downloadCustomArgs"),
                      settings.value(QStringLiteral("downloadCustomArgs")).toString().trimmed());
    normalized.insert(QStringLiteral("embedMetadata"),
                      settings.value(QStringLiteral("embedMetadata"), true).toBool());
    normalized.insert(QStringLiteral("embedThumbnail"),
                      settings.value(QStringLiteral("embedThumbnail"), true).toBool());
    normalized.insert(QStringLiteral("cropCoverArt"),
                      settings.value(QStringLiteral("cropCoverArt"), false).toBool());
    normalized.insert(QStringLiteral("removeSourceMetadata"),
                      settings.value(QStringLiteral("removeSourceMetadata"), false).toBool());
    normalized.insert(QStringLiteral("useAria2c"),
                      settings.value(QStringLiteral("useAria2c"), false).toBool());
    normalized.insert(QStringLiteral("aria2cMaxConnections"),
                      qBound(1, settings.value(QStringLiteral("aria2cMaxConnections"), 16).toInt(), 16));
    normalized.insert(QStringLiteral("aria2cMinSplitSizeMb"),
                      qBound(1, settings.value(QStringLiteral("aria2cMinSplitSizeMb"), 20).toInt(), 100));
    return normalized;
}
