    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            RowLayout {
                ...
                Button {
                    id: swatchButton
                    implicitWidth: Math.round(36 * UiMetrics.fontScale)
                    implicitHeight: UiMetrics.controlHeightCompact
                    enabled: root.isEffectivelyEnabled
                    activeFocusOnTab: true
                    Accessible.name: root.title + " " + root.colorValue.toString()
                    onClicked: root.chooseColorClicked()

                    background: Rectangle {
                        radius: themeManager.borderRadius
                        color: root.colorValue
                        border.width: 1
                        border.color: swatchButton.activeFocus ? themeManager.primaryColor : themeManager.borderColor
                    }
                }
            }
        }
